...
export default function Header({ 
  ...
  return (
    <header 
      className="sticky top-0 z-50 bg-white dark:bg-gray-800 dark:bg-gray-900 border-b border-[#F3F4F6] dark:border-gray-700 shadow-sm"
      dir={isRTL ? "rtl" : "ltr"}
    >
      <div className="max-w-7xl mx-auto px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {showBackButton && (
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white dark:bg-gray-800 border border-[#F3F4F6] dark:border-gray-700 text-[#111827] dark:text-gray-100 hover:bg-[#F3F4F6] dark:hover:bg-gray-700 hover:scale-105 active:scale-95 transition-all shadow-sm"
                aria-label={t("back")}
              >
                {isRTL ? (
          ...
          {/* Titre (optionnel) */}
          {title && (
            <h1 className="text-lg font-bold text-[#111827] dark:text-gray-100 truncate px-4">
              {title}
            </h1>
          )}
