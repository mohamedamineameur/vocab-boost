  ...
  return (
    <button
      ...
      className={[
        ...
        selected ? "bg-[#22C55E] text-white border-transparent" : "bg-white dark:bg-gray-800 text-[#111827] border-black/10 hover:border-[#3B82F6]",
...
function TokenSorter({ target, initial, onChange }: { target: string; initial: string; onChange: (value: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2 min-h-[40px] rounded-xl border border-black/10 bg-white dark:bg-gray-800 p-2">
        {picked.map((tok, i) => (
      ...
      <div className="flex items-center gap-2">
        <button onClick={reset} className="inline-flex items-center gap-2 rounded-xl border border-black/10 bg-white dark:bg-gray-800 px-3 py-1.5 text-sm hover:shadow"><RotateCcw className="h-4 w-4"/> {i18n.en.reset}</button>
...
export default function QuizPage() {
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      console.error("Erreur lors de la mise à jour du quiz:", e);
    }
  ...
  const renderQuizBody = (q: Quiz) => {
    switch (q.type) {
        ...
        return (
          <div className="space-y-3">
            <input
              ...
              className="w-full rounded-2xl border border-black/10 bg-white dark:bg-gray-800 px-4 py-3 outline-none focus:ring-2 focus:ring-[#3B82F6]"
            />
      ...
      case 'speaking':
        return (
          <div className="space-y-3">
            <div className="rounded-2xl border border-black/10 bg-white dark:bg-gray-800 p-3 min-h-[56px]">
  ...
  const renderTTSButtons = (q: Quiz) => (
    <div className="flex items-center gap-2">
      <button onClick={() => handleSpeak(questionFor(q))} className="inline-flex items-center gap-2 rounded-xl bg-white dark:bg-gray-800 border border-black/10 px-3 py-1.5 text-sm hover:shadow"><Volume2 className="h-4 w-4"/>{i18n.en.tts}</button>
      <button onClick={() => stop()} className="inline-flex items-center gap-2 rounded-xl bg-white dark:bg-gray-800 border border-black/10 px-3 py-1.5 text-sm hover:shadow"><VolumeX className="h-4 w-4"/>{i18n.en.stopTts}</button>
    </div>
  );
      ...
      <div className="min-h-screen flex flex-col bg-gradient-to-b from-white to-[#F3F4F6] text-[#111827]">
        <header className="px-5 pt-6 pb-3 sticky top-0 z-20 bg-white dark:bg-gray-800/80 backdrop-blur border-b border-black/5">
          <div className="max-w-3xl mx-auto flex items-center justify-between">
            <button onClick={() => navigate(-1)} className="inline-flex items-center gap-2 rounded-2xl border border-black/10 px-3 py-2 text-sm bg-white dark:bg-gray-800"><ChevronLeft className="h-4 w-4"/>{t('back')}</button>
            <div className="text-sm text-black/60">{t('title')}</div>
          </div>
        </header>
        <main className="flex-1">
          <div className="max-w-3xl mx-auto px-5 py-10">
            <div className="rounded-2xl border border-dashed border-black/10 bg-white dark:bg-gray-800 p-6 text-center">
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-white to-[#F3F4F6] text-[#111827]">
      {/* Header */}
      <header className="px-5 pt-6 pb-3 sticky top-0 z-30 bg-white dark:bg-gray-800/80 backdrop-blur border-b border-black/5">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <button onClick={() => navigate(-1)} className="inline-flex items-center gap-2 rounded-2xl border border-black/10 px-3 py-2 text-sm bg-white dark:bg-gray-800"><ChevronLeft className="h-4 w-4"/>{t('back')}</button>
      ...
      <main className="flex-1">
        <div className="mx-auto max-w-3xl px-5 py-6">
          <div className="rounded-2xl bg-white dark:bg-gray-800 border border-black/5 shadow-md p-5">
            ...
            <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-2xl py-3 px-6 shadow-md bg-white dark:bg-gray-800 border border-black/10 hover:shadow"
              >
